...
const uiState = {
  ...
  watchlistAddDebug:{},
  reviewWorkspaceMessages:{},
  reviewWorkspaceDelegatesBound:false,
...
function renderReviewWatchlistAddDebugPane(record){
  const item = normalizeTickerRecord(record || {});
  const debug = item.ticker ? (uiState.watchlistAddDebug[item.ticker] || {}) : {};
  const lifecycle = syncWatchlistLifecycle(item) || watchlistLifecycleSnapshot(item);
  const watchlistEntry = tickerRecordToWatchlistEntry(item);
  const hiddenByDefaultFilter = !state.showExpiredWatchlist && ['dead','expired'].includes(String(lifecycle.state || ''));
  return `<details class="compact-details"><summary>Watchlist Add Debug</summary><div class="mutebox scrollbox">ticker: ${escapeHtml(item.ticker || '(none)')}\ncanonical_key: ${escapeHtml(item.ticker || '(none)')}\nin_watchlist: ${escapeHtml(String(!!(item.watchlist && item.watchlist.inWatchlist)))}\nwatchlist_entry_exists: ${escapeHtml(String(!!watchlistEntry))}\nlast_add_result: ${escapeHtml(String(debug.result || '(none)'))}\nlast_add_source: ${escapeHtml(String(debug.source || '(none)'))}\nlast_add_at: ${escapeHtml(String(debug.at || '(none)'))}\nlast_add_message: ${escapeHtml(String(debug.message || '(none)'))}\nstate.showExpiredWatchlist: ${escapeHtml(String(!!state.showExpiredWatchlist))}\nvisible_in_watchlist_default_filter: ${escapeHtml(String(!hiddenByDefaultFilter && !!watchlistEntry))}\nhidden_reason: ${escapeHtml(hiddenByDefaultFilter ? `filtered_${String(lifecycle.state || 'unknown')}` : 'visible')}\nwatchlist_lifecycle_state: ${escapeHtml(String(lifecycle.state || '(none)'))}\nwatchlist_bucket: ${escapeHtml(String(lifecycle.bucket || '(none)'))}\nwatchlist_status: ${escapeHtml(String(lifecycle.status || '(none)'))}\nwatchlist_stage: ${escapeHtml(String(lifecycle.stage || '(none)'))}\nwatchlist_reason: ${escapeHtml(String(lifecycle.reason || '(none)'))}\nwatchlist_added_at: ${escapeHtml(String(item.watchlist && item.watchlist.addedAt || '(none)'))}\nwatchlist_updated_at: ${escapeHtml(String(item.watchlist && item.watchlist.updatedAt || '(none)'))}</div></details>`;
}
...
function buildFinalSetupView(record, options = {}){
  ...
  return {
    ...
    setupUiState:{
      ...finalSetupUiState,
      label:scannerResolution.status,
      className:primaryVerdictBadge(scannerResolution.status).className
    },
    setupState:scannerResolution.setupState,
    setupLabel:scannerResolution.status,
    rrCategory,
    structureQuality,
    isStructureValid,
    hasPlanAdjustmentBlock,
    isNotReadySetup,
    scannerResolution,
    displayStage:scannerResolution.status,
    finalVerdict:scannerResolution.status,
    finalClassification,
    bucket,
...
function renderScannerDecisionTrace(view){
...
function actionPresentationForRecord(record, options = {}){
  ...
  if(reviewVerdict === 'Avoid' && avoidSubtype === 'conditional'){
    ...
    if(volumeState === 'weak' && bounceState === 'confirmed'){
      return {
        label:'Monitor for stronger volume confirmation',
        tone:'warning-soft',
        shortLabel:'Monitor for stronger volume confirmation'
      };
    }
    return {
  ...
  if(reviewVerdict === 'Entry'){
    return {
      label:'✅ Execute trade plan',
      tone:'success',
      shortLabel:'Execute trade plan'
    };
  }

  if(reviewVerdict === 'Near Entry'){
    return {
      label:'🛠 Prepare entry - near trigger',
      tone:'warning',
      shortLabel:'Prepare entry'
    };
  }
...
function addActiveReviewTickerToWatchlist(){
  const ticker = activeReviewTicker();
  if(!ticker){
    const missingMarkup = '<span class="warntext">Select a ticker in the review workspace first.</span>';
    setReviewWorkspaceMessage(ticker, missingMarkup);
    setStatus('reviewWorkspaceStatus', missingMarkup);
    return;
  }
  const liveRecord = upsertTickerRecord(ticker);
  const beforeInWatchlist = !!(liveRecord.watchlist && liveRecord.watchlist.inWatchlist);
  ...
  const updatedRecord = getTickerRecord(ticker) || liveRecord;
  const lifecycle = syncWatchlistLifecycle(updatedRecord) || watchlistLifecycleSnapshot(updatedRecord);
  const hiddenByDefaultFilter = !state.showExpiredWatchlist && ['dead','expired'].includes(String(lifecycle.state || ''));
  uiState.watchlistAddDebug[ticker] = {
    ...(uiState.watchlistAddDebug[ticker] || {}),
    source:'review_workspace',
    at:new Date().toISOString(),
    result:result && result.error ? 'error' : (beforeInWatchlist ? 'updated' : 'added'),
    message:result && result.error
      ? `Could not add ${ticker} to the watchlist.`
      : (hiddenByDefaultFilter
        ? `${ticker} was written to the watchlist but is hidden by the current watchlist filter.`
        : (beforeInWatchlist
          ? `${ticker} was already in the watchlist and was updated.`
          : `${ticker} was added to the watchlist.`))
  };
  renderReviewLifecycleSummary(ticker);
  if(result && result.error){
    const errorMarkup = `<span class="warntext">Could not add ${escapeHtml(ticker)} to the watchlist.</span>`;
    setReviewWorkspaceMessage(ticker, errorMarkup);
    setStatus('reviewWorkspaceStatus', errorMarkup);
    renderReviewWorkspace();
    return;
  }
  const statusMarkup = hiddenByDefaultFilter
    ? `<span class="ok">${escapeHtml(ticker)} saved to the watchlist, but it is currently hidden by the watchlist filter.</span>`
  ...
  setReviewWorkspaceMessage(ticker, statusMarkup);
...
function bindPersistentReviewWorkspaceDelegates(){
  if(uiState.reviewWorkspaceDelegatesBound) return;
  const box = $('reviewWorkspace');
  if(!box) return;
  box.addEventListener('click', event => {
    ...
    if(target.matches('#analyseActiveBtn')){
      event.preventDefault();
      event.stopPropagation();
      analyseActiveReviewTicker();
      return;
    }
    if(target.matches('#saveReviewBtn')){
      event.preventDefault();
      event.stopPropagation();
      saveReview();
      return;
    }
    if(target.matches('#addWatchlistActiveBtn')){
      event.preventDefault();
      event.stopPropagation();
      const ticker = activeReviewTicker();
      if(ticker){
        uiState.watchlistAddDebug[ticker] = {
          ...
          at:new Date().toISOString(),
          result:'clicked',
          message:`Add to Watchlist clicked for ${ticker}.`
        };
      }
      addActiveReviewTickerToWatchlist();
      return;
    }
    if(target.matches('#resetReviewBtn')){
      event.preventDefault();
      event.stopPropagation();
      resetReview();
      return;
    }
    if(target.matches('#expireLifecycleBtn')){
      event.preventDefault();
      event.stopPropagation();
      expireSelectedTickerLifecycle();
    }
  });
  uiState.reviewWorkspaceDelegatesBound = true;
}

function handleReviewWorkspaceAction(action){
  const normalizedAction = String(action || '').trim().toLowerCase();
  if(!normalizedAction) return false;
  if(normalizedAction === 'analyse'){
    analyseActiveReviewTicker();
    return false;
  }
  ...
  if(normalizedAction === 'add-watchlist'){
    const ticker = activeReviewTicker();
    if(ticker){
      uiState.watchlistAddDebug[ticker] = {
        ...(uiState.watchlistAddDebug[ticker] || {}),
...
window.handleReviewWorkspaceAction = handleReviewWorkspaceAction;

function setReviewWorkspaceMessage(ticker, markup){
  const symbol = normalizeTicker(ticker);
  if(!symbol) return;
  uiState.reviewWorkspaceMessages[symbol] = String(markup || '');
}

function reviewWorkspaceStatusMarkup(record, loading, analysisBusy){
  const ticker = normalizeTicker(record && record.ticker);
  if(ticker && uiState.reviewWorkspaceMessages[ticker]){
    return uiState.reviewWorkspaceMessages[ticker];
  }
  return renderCardStatusLineFromRecord(record, loading, analysisBusy);
}
...
function renderReviewWorkspace(options = {}){
  ...
  const reviewBadgeLabel = emojiPresentation.primaryText;
  ...
  const modifierMarkup = emojiModifierMarkup(emojiPresentation);
  ...
  if(scannerPresentation.primaryText !== emojiPresentation.primaryText){
    console.warn('EMOJI_SURFACE_DISAGREEMENT', {ticker:record.ticker, scanner:scannerPresentation.primaryText, review:emojiPresentation.primaryText});
  }
  ...
  const reviewReasonText = reviewReasonSummary(decisionReasoning, nextActionText);
  ...
  if((displayStage !== 'Avoid' && emojiPresentation.primaryState === 'dead') || (displayStage === 'Avoid' && emojiPresentation.primaryState !== 'dead' && avoidSubtype === 'terminal')){
  ...
  const reviewWatchlistDebug = renderReviewWatchlistAddDebugPane(record);
  ...
  box.innerHTML = `<div class="reviewworkspace ready"><div class="reviewhero reviewhero-compact"><div class="reviewherohead"><div class="reviewheadline"><div class="inline-status"><strong>${escapeHtml(record.ticker)}</strong><span class="badge ${emojiPresentation.badgeClass}">${escapeHtml(reviewBadgeLabel)}</span><span class="score ${scoreClass(setupScore)}">${escapeHtml(setupScoreDisplay)}</span>${modifierMarkup}${headerContextMarkup}</div><div class="tiny">${escapeHtml(companyLine)}</div>${downgradeSummary ? `<div class="tiny review-downgrade-badge"><span class="badge avoid">${escapeHtml(downgradeSummary.label)}</span> ${escapeHtml(downgradeSummary.transition)}</div>` : ''}<div class="tiny review-next-action review-next-action-primary">${escapeHtml(nextActionText)}</div>${reviewReasonText ? `<div class="tiny review-decision-detail">${escapeHtml(reviewReasonText)}</div>` : ''}</div></div>${marketMetaLine}</div><div class="panelbox reviewchartpanel"><div class="reviewsectionhead"><strong>Chart</strong><div class="workflowmenu workflowmenu-inline"><button class="secondary compactbutton" type="button" data-act="open-chart">Open Chart</button><button class="secondary compactbutton" type="button" data-act="choose-chart">Choose Screenshot</button><button class="secondary compactbutton" type="button" data-act="import-latest">Import Latest</button><button class="ghost compactbutton" type="button" data-act="clear-chart">Remove Chart</button></div></div><input id="reviewChartFile" type="file" accept="image/png,image/jpeg,image/*" hidden />${chartPreview}</div><div class="panelbox reviewanalysispanel"><div class="reviewsectionhead"><strong>Analyse Setup</strong><span class="tiny">Run AI analysis before reviewing saved notes and planning.</span></div><div class="reviewactions reviewactions-top"><button class="primary" id="analyseActiveBtn" type="button" onclick="return window.handleReviewWorkspaceAction('analyse')" ${analysisBusy ? 'disabled' : ''}>${escapeHtml(analyseLabel)}</button><button class="ghost" id="resetReviewBtn" type="button" onclick="return window.handleReviewWorkspaceAction('remove')">Remove</button></div></div><div class="panelbox"><div class="reviewsectionhead"><strong>AI Analysis + Notes</strong><span class="tiny">Prompt and debug stay collapsed until needed.</span></div><textarea id="reviewNotes" placeholder="Add ticker-specific notes here.">${escapeHtml(record.review.notes || '')}</textarea><details class="responsepanel compact-open-on-demand" id="reviewResponse" ${(((uiState.responseOpen[record.ticker] ?? false) || !!analysisState.error)) ? 'open' : ''}><summary>Saved AI Read</summary>${renderAnalysisPanelFromRecord(record)}</details><details class="promptdetails compact-open-on-demand" id="reviewPrompt" ${(uiState.promptOpen[record.ticker] ?? false) ? 'open' : ''}><summary>Prompt Preview</summary><div class="mutebox scrollbox">${escapeHtml(promptText)}</div></details></div><div class="panelbox"><strong id="plannerSection">Trade Plan</strong><div class="summary" id="plannerPlanSummary">Entry: Not given | Stop: Not given | First Target: Not given | Planned R:R: N/A</div><input id="selectedTicker" value="${escapeHtml(record.ticker)}" readonly hidden /><div class="plan-grid plan-grid-inputs"><div><label>Planned Entry</label><input id="entryPrice" type="number" step="0.01" value="${escapeHtml(effectivePlan.entry || '')}" /></div><div><label>Planned Stop</label><input id="stopPrice" type="number" step="0.01" value="${escapeHtml(effectivePlan.stop || '')}" /></div><div><label>${escapeHtml(executionState.exitMode === 'dynamic_exit' ? 'Target Review Level' : 'Planned First Target')}</label><input id="targetPrice" type="number" step="0.01" value="${escapeHtml(effectivePlan.firstTarget || '')}" /></div></div><div class="reviewstats plan-grid plan-grid-stats"><div class="stat"><div>Risk / Share</div><div class="big" id="riskPerShare">-</div></div><div class="stat"><div>Reward / Share</div><div class="big" id="rewardPerShareBox">${escapeHtml(Number.isFinite(rewardPerShare) ? rewardPerShare.toFixed(2) : '-')}</div></div><div class="stat"><div>Raw R:R</div><div class="big" id="rrValue">${escapeHtml(rawRrDisplay)}</div></div><div class="stat"><div>Position Size</div><div class="big" id="positionSize">-</div></div><div class="stat"><div>Position Cost</div><div class="big" id="positionCostBox">${escapeHtml(positionCostText)}</div></div><div class="stat"><div>Max Loss</div><div class="big">${escapeHtml(formatGbp(currentMaxLoss()))}</div></div><div class="stat"><div>Risk / Capital</div><div class="big" id="riskFitBox">${escapeHtml(`${riskStatusLabel(record.plan.riskStatus || 'plan_missing')} / ${capitalComfort.label}`)}</div></div><div class="stat"><div>Capital Check</div><div class="big" id="capitalCheckBox">${escapeHtml(capitalComfort.note || 'Clear')}</div></div></div><div class="summary" id="planRealismSummary">${escapeHtml(planRealismSummary)}</div><div class="tiny" id="planRealismReasons">${escapeHtml(planRealismReasons)}</div><details class="compact-details"><summary>Plan Diagnostics</summary><div class="reviewmeta-grid"><div><label>Plan State</label><input id="planStateBox" readonly value="${escapeHtml(planState)}" /></div><div><label>Plan Math</label><input id="planQualityBox" readonly value="${escapeHtml(planQualityForRr(rrRatio) || 'N/A')}" /></div><div><label>RR Realism</label><input id="rrRealismBox" readonly value="${escapeHtml(planRealism.rr_realism_label)}" /></div><div><label>Credible RR</label><input id="credibleRrBox" readonly value="${escapeHtml(credibleRrDisplay)}" /></div><div><label>Optimistic Target</label><input id="optimisticTargetBox" readonly value="${escapeHtml(planRealism.optimistic_target_flag ? 'Yes' : 'No')}" /></div><div><label>Target Assessment</label><input id="targetAssessmentBox" readonly value="${escapeHtml(planRealism.credible_target_assessment)}" /></div><div><label>Plan Source</label><input id="planSourceBox" readonly value="${escapeHtml(String(record.plan.source || effectivePlan.source || ''))}" /></div><div><label>Trigger State</label><input id="triggerStateBox" readonly value="${escapeHtml(triggerStateLabel(record.plan.triggerState))}" /></div><div><label>Plan Check</label><input id="planValidationBox" readonly value="${escapeHtml(planValidationStateLabel(planCheckState))}" /></div><div><label>Execution Mode</label><input id="exitModeBox" readonly value="${escapeHtml(executionModeText)}" /></div><div><label>Target Review</label><input id="targetReviewStateBox" readonly value="${escapeHtml(targetReviewText)}" /></div><div><label>Target Alert</label><input id="targetAlertBox" readonly value="${escapeHtml(targetAlertText)}" /></div><div><label>Capital Comfort</label><input id="capitalComfortBox" readonly value="${escapeHtml(capitalComfort.label)}" /></div></div></details><div class="summary">${escapeHtml(executionState.exitMode === 'dynamic_exit' ? `${targetGuidance} | ${targetReviewText} | ${targetActionText}` : 'Target order can be managed as a fixed exit.')}</div><div class="tiny" id="calcNote">Enter planned entry, stop, and first target to calculate size.</div></div><details class="panelbox reviewchecklist"><summary><strong>Checklist</strong> <span id="progressText">Checks met: 0 / 10</span></summary><div class="prog"><div class="fill" id="progressFill"></div></div><div class="checks"><div class="checkgroup"><h3>Trend</h3><label class="checkitem"><input type="checkbox" class="logic" id="trendStrong" ${reviewChecks.trendStrong ? 'checked' : ''}> Strong uptrend</label><label class="checkitem"><input type="checkbox" class="logic" id="above50" ${reviewChecks.above50 ? 'checked' : ''}> Above 50 MA</label><label class="checkitem"><input type="checkbox" class="logic" id="above200" ${reviewChecks.above200 ? 'checked' : ''}> Above 200 MA</label><label class="checkitem"><input type="checkbox" class="logic" id="ma50gt200" ${reviewChecks.ma50gt200 ? 'checked' : ''}> 50 MA above 200 MA</label></div><div class="checkgroup"><h3>Pullback + Confirmation</h3><label class="checkitem"><input type="checkbox" class="logic" id="near20" ${reviewChecks.near20 ? 'checked' : ''}> Near 20 MA</label><label class="checkitem"><input type="checkbox" class="logic" id="near50" ${reviewChecks.near50 ? 'checked' : ''}> Near 50 MA</label><label class="checkitem"><input type="checkbox" class="logic" id="stabilising" ${reviewChecks.stabilising ? 'checked' : ''}> Stabilising</label><label class="checkitem"><input type="checkbox" class="logic" id="bounce" ${reviewChecks.bounce ? 'checked' : ''}> Bounce candle</label><label class="checkitem"><input type="checkbox" class="logic" id="volume" ${reviewChecks.volume ? 'checked' : ''}> Volume supportive</label></div><div class="checkgroup"><h3>Trade Plan</h3><label class="checkitem"><input type="checkbox" class="logic" id="entryDefined" ${reviewChecks.entryDefined ? 'checked' : ''}> Entry defined</label><label class="checkitem"><input type="checkbox" class="logic" id="stopDefined" ${reviewChecks.stopDefined ? 'checked' : ''}> Stop defined</label><label class="checkitem"><input type="checkbox" class="logic" id="targetDefined" ${reviewChecks.targetDefined ? 'checked' : ''}> Target defined</label></div></div><div class="summary" id="summaryBox">No setup reviewed yet.</div></details><div class="reviewactions"><button class="secondary" id="saveReviewBtn" type="button" onclick="return window.handleReviewWorkspaceAction('save')">Save Review</button><button class="secondary" id="addWatchlistActiveBtn" type="button" onclick="return window.handleReviewWorkspaceAction('add-watchlist')">Add to Watchlist</button></div><details class="panelbox compact-details"><summary>Workspace Status</summary><div class="summary" id="reviewLifecycleSummary">Lifecycle: Not tracked yet.</div><div class="reviewactions reviewactions-secondary"><button class="ghost" id="expireLifecycleBtn" type="button" onclick="return window.handleReviewWorkspaceAction('expire')">Expire Now</button></div>${reviewDebug}${reviewWatchlistDebug}<div class="statusline tiny" id="reviewWorkspaceStatus">${reviewWorkspaceStatusMarkup(record, loading, analysisBusy)}</div></details></div>`;
...
bindPersistentReviewWorkspaceDelegates();
updateTickerSearchStatus();
updateProviderStatusNote();
