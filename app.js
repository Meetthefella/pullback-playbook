...
const {
  watchlistLifecycleSnapshot: watchlistLifecycleSnapshotImpl
} = window.WatchlistLifecycleSnapshot;
const {
...
function watchlistLifecycleStateRank(state){
  return watchlistLifecycleStateRankImpl(state);
}

function canonicalLifecycleState(state){
  return canonicalLifecycleStateImpl(state);
}

function resolveLifecycleTransition(currentState, inputs = {}){
  return resolveLifecycleTransitionImpl(currentState, inputs, {
...
function watchlistLifecycleSnapshot(record){
  return watchlistLifecycleSnapshotImpl(record, {
    normalizeTickerRecord,
    ...
    WATCHLIST_EXPIRY_TRADING_DAYS,
    PLAN_EXPIRY_TRADING_DAYS
  });
}
