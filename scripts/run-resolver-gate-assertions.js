...
function runReviewCommittedTracePersistenceAssertions(){
  const recordsSource = fs.readFileSync(path.join(root, 'js/records.js'), 'utf8');
  const recordsSandbox = {
    window: {},
    console:{
      ...console,
      info(){},
  ...
  recordsSandbox.globalThis = recordsSandbox.window;
  vm.runInNewContext(recordsSource, recordsSandbox, {filename:path.join(root, 'js/records.js')});
  const appRecords = recordsSandbox.window.AppRecords;
  if(!appRecords || typeof appRecords.normalizeTickerRecord !== 'function'){
    throw new Error('Review record normalization helper is unavailable.');
  }
  const deps = {
    normalizeTicker(value){ return String(value || '').trim().toUpperCase(); },
    createBaseTickerRecord(ticker){
      return {
        ...
        review:{
          ...
          savedSummary:'',
          savedScore:null,
          analysisState:{raw:'', normalized:null, prompt:'', error:'', reviewedAt:''},
          aiAnalysisRaw:'',
        ...
        plan:{
          ...
          targetAlert:{enabled:true, level:null, lastState:''},
          riskPerShare:null,
          rewardPerShare:null,
          plannedRR:null,
          positionSize:null,
        ...
        diary:{records:[]},
        lifecycle:{stage:'', status:'inactive', expiresAt:'', changedAt:'', source:''},
        meta:{}
      };
    },
    normalizeScanType(value){ return String(value || ''); },
    numericOrNull(value){
      if(value === null || value === undefined || value === '') return null;
      const numeric = Number(value);
      return Number.isFinite(numeric) ? numeric : null;
    },
    normalizeImportedStatus(value){ return String(value || ''); },
    normalizeExitMode(value){ return String(value || ''); },
    normalizeTargetReviewState(value){ return String(value || ''); },
    normalizeStoredPlanSnapshot(value){ return value && typeof value === 'object' ? value : null; },
    normalizeTradeRecord(value){ return value && typeof value === 'object' ? value : {}; },
    uniqueStrings(values){ return [...new Set((values || []).map(value => String(value || '').trim()).filter(Boolean))]; },
    normalizeStoredTradeOutcome(value){ return value && typeof value === 'object' ? value : {}; },
    hasAnyPlanFields(){ return false; },
    deriveExecutionPlanState(){ return {}; },
    analysisDerivedStatesFromRecord(){ return {}; },
    computeBaseSetupScoreForRecord(){ return null; },
    deriveCurrentPlanState(){ return {}; },
    evaluateSetupQualityAdjustments(){ return {controlQuality:'', capitalEfficiency:'', adjustmentReasons:[]}; },
    warningStateFromInputs(){ return {}; },
    deriveDisplaySetupScore(){ return 0; },
    convictionTierForRecord(){ return {}; },
    practicalSizeFlagForPlan(){ return {}; },
    evaluateEntryTrigger(){ return {}; },
    validateCurrentPlan(){ return {state:'', needsReplan:false, missed:false, invalidated:false}; },
    hasLockedLifecycle(){ return false; },
    deriveActionStateForRecord(){ return {}; },
    state:{marketStatus:''}
  };
  const committedTrace = {
    phase:'merged',
    status:'consistent',
    mergedStatus:'consistent',
    renderedStatus:'consistent',
    requestId:'analysis-1-1779192664830',
    verificationRequestId:'analysis-1-1779192664830',
    imageId:'img_a388019c_350338',
    chartImageId:'img_a388019c_350338',
    trace:{status:'consistent', mergedStatus:'consistent', phase:'merged'}
  };
  const firstPass = appRecords.normalizeTickerRecord({
    ticker:'LIN',
    review:{
      chartVerificationCommittedTrace:committedTrace
    }
  }, deps);
  if(!firstPass.review || !firstPass.review.chartVerificationCommittedTrace || firstPass.review.chartVerificationCommittedTrace.status !== 'consistent'){
    throw new Error('Committed trace must survive the first review normalization pass.');
  }
  const secondPass = appRecords.normalizeTickerRecord(firstPass, deps);
  if(!secondPass.review || !secondPass.review.chartVerificationCommittedTrace || secondPass.review.chartVerificationCommittedTrace.status !== 'consistent'){
    throw new Error('Committed trace must survive a review rebuild round-trip.');
  }
}

runReviewCommittedTracePersistenceAssertions();
